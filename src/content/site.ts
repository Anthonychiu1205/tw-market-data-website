...
export const datasetProducts: DatasetProduct[] = [
  ...
  { id: "institutional-flow", name: "institutional-flow", domain: "籌碼與資金", marketCoverage: "TWSE 法人買賣", maturity: "v2", readiness: "available_now", sourceOrigin: "TWSE official T86", shortUseCase: "TWSE 三大法人買賣超與淨流量", endpoint: "/v2/datasets/institutional-flow" },
  // readiness "preview" (not "available_now"): the read-API route is still private_beta and returns 404,
  // so this must not render as a verified release-grade product (backend-adapter maps available_now →
  // isReleaseGrade). The DATA is backend-verified (n6 green); only the endpoint is not open yet. Flip back
  // to "available_now" when the backend route goes production (tw-feature-engine flip PR) — see DOCS ticket.
  { id: "securities-lending", name: "securities-lending", domain: "籌碼與資金", marketCoverage: "TWSE 借券資料", maturity: "v2", readiness: "preview", sourceOrigin: "TWSE official TWT72U", shortUseCase: "TWSE 借券餘額、借入、還券與 known gaps", endpoint: "/v2/datasets/securities-lending" },
