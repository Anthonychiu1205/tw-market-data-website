...
// the request examples so they can never disagree. Authority (owner ruling): backend
// /openapi.json drives param name / required / type for the endpoints it documents; the
// verified fixture drives the required-filter NAMES (index_code / rate_code / series_id /
// country / dataset_name, ticker vs symbol) for endpoints openapi does not cover. Date params
// use the openapi names start_date/end_date (the fixture's date_from is ignored by the backend —
// start_date returns the correctly-filtered range). Do NOT hand-edit; re-vendor from source.
